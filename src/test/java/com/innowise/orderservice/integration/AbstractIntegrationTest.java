...
public abstract class AbstractIntegrationTest {
    ...
    protected HttpHeaders adminHeaders(String method, String path) {
        return buildHeaders(1L, "admin@example.com", Role.ADMIN, method, path);
    }

    protected HttpHeaders userHeaders(Long userId, String email, String method, String path) {
        return buildHeaders(userId, email, Role.USER, method, path);
    }
    ...
    private HttpHeaders buildHeaders(Long userId, String email, Role role, String method, String path) {
        HttpHeaders headers = new HttpHeaders();
        String ts = String.valueOf(Instant.now().getEpochSecond());
        String signature = sign(buildPayload(method, path, String.valueOf(userId), role.name(), email, email, ts));
